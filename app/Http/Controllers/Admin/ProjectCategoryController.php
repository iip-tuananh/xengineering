...
class ProjectCategoryController extends Controller
{
    ...
    {
        $categories = ThisModel::query()->where('type', PostCategory::TYPE_PROJECT)
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $object->type = PostCategory::TYPE_PROJECT;
    ...
    {
        $object = ThisModel::findOrFail($id);
        if (!$object->canDeleteProject()) {
            $message = array(
                "message" => "Không thể xóa. Đã có dự án đã sử dụng danh mục này!",
                "alert-type" => "warning"
            );
        } else {
            if($object->image) {
